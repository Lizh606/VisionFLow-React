...
import { ThemeColors, Language, ThemeMode } from '../../../types/index';
                ...
                <div className="p-5 space-y-5 bg-blue-500/5">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs">
                        <div className="p-3 rounded-lg border bg-surface/50" style={{ borderColor: theme.stroke, background: theme.surface }}>
                            <div className="font-bold mb-1 opacity-70" style={{ color: theme.textSecondary }}>{t.summary.billingDesc}</div>
                            <div className="font-medium" style={{ color: theme.text }}>
                                {selectedLicense.billingMode === 'PER_DEVICE' ? 'Billed per active device. Monthly cap applies.' : 'Billed per runtime hour.'}
                            </div>
                        </div>
                         <div className="p-3 rounded-lg border bg-surface/50" style={{ borderColor: theme.stroke, background: theme.surface }}>
                            <div className="font-bold mb-1 opacity-70" style={{ color: theme.textSecondary }}>{t.summary.capabilities}</div>
                            <div className="font-medium" style={{ color: theme.text }}>
                                {selectedLicense.offlineLease ? 'Offline Lease • ' : ''}
                                {selectedLicense.total > 0 ? `${selectedLicense.total} Max Devices` : 'Unlimited Devices'}
                            </div>
                        </div>
                    </div>
                </div>
            ) : (
                <div className="p-5 flex items-center justify-center opacity-50 italic text-sm">
                    Select a license from the list above to view details.
                </div>
            )}
            
            <div className="p-5 border-t flex justify-end gap-3" style={{ borderColor: theme.stroke, background: theme.surface }}>
                <button 
                  onClick={onClose}
                ...
                <button 
                  ...
                  className="px-8 py-2.5 rounded-xl font-bold text-sm text-white shadow-lg shadow-blue-500/30 hover:scale-105 active:scale-95 transition-all bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed"
                >
                  {t.confirm}
                </button>
            </div>
