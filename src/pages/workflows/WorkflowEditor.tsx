...
import { Inspector, RightPanelView } from '../../features/workflow/components/Inspector';
import { WorkflowCanvas } from '../../features/workflow/components/WorkflowCanvas';
import { NodeLibraryPopover } from '../../features/workflow/components/NodeLibraryModal'; 
import { ModelLibraryModal } from '../../features/workflow/components/ModelLibraryModal';
import { HistorySidebar } from '../../features/workflow/components/HistorySidebar';
import { INITIAL_NODES, INITIAL_CONNECTIONS } from '../../mocks';
import { ThemeColors, ThemeMode, Node, NodeType, Connection, Language } from '../../types/index';
import { BrainCircuit, Layers, History, ChevronRight } from 'lucide-react';
        ...
        <div className="flex-1 relative h-full flex flex-col min-w-0">
           ...
           >
               <button
                  ...
                  className={`flex items-center gap-2 px-2 py-2 sm:px-3 sm:py-2 rounded-xl shadow-lg border backdrop-blur-md transition-all group hover:scale-105 active:scale-95 ${isHistoryOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
                  style={{ 
               ...
               >
                   ...
                   <span className="font-semibold text-xs hidden sm:inline">History</span>
                   <ChevronRight size={14} className="opacity-40 group-hover:translate-x-0.5 transition-transform" />
               </button>
           </div>

           <div 
             className="absolute top-6 right-4 sm:right-6 flex flex-col gap-3 z-20 transition-all duration-300"
           >
                 <button
                   onClick={() => setActivePanel(activePanel === 'nodes' ? null : 'nodes')}
                   className={`flex items-center justify-center sm:justify-start gap-2 w-10 h-10 sm:w-auto sm:h-auto sm:px-4 sm:py-3 rounded-full font-semibold shadow-xl transition-all duration-200 group border ${activePanel === 'nodes' ? 'ring-2 ring-offset-2 ring-blue-500' : 'hover:scale-105'}`}
                   style={{ 
                     background: theme.surface, 
                     color: theme.text,
                     borderColor: theme.stroke,
                 ...
                 <button
                   ...
                   className="flex items-center justify-center sm:justify-start gap-2 w-10 h-10 sm:w-auto sm:h-auto sm:px-4 sm:py-3 rounded-full font-semibold shadow-xl transition-all duration-200 group border hover:scale-105"
                   style={{ 
                     background: theme.surface, 
                     color: theme.text,
                     borderColor: theme.stroke
